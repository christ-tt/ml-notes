...
Sadly, we show that on PP can’t make good use of GPUs with high P2P.

See section **A Tension: Batch Size, Stage Depth, and How Much Memory We Actually Touch**

Working on showing why on MAC this might be the case.
...
## **Why Focus on GEMM (and Specifically MLP / Attention GEMMs)?**
...
## **Concrete setup: Qwen2-32B on RTX 4090 (decode vs prefill)**
...
Let’s try to explain why the critical stage alone determines the overall throughput, without any hard math.
...
### Number of Stages, Micro-Batches, and Bubbles
...
- We can only **evict** KV entries once generation is finished (token by token).
- **VRAM is finite**, so the total number of KV tokens we can store is bounded.
- System-wide, the number of active sequences in flight at any step is bounded by the **smallest per-stage KV capacity** (in sequences).
...
- The **minimum microbatch size** is 1, so the **maximum number of non-empty microbatches** is 16.
...
# Pipeline Parallel - Analyzing Throughput
...
Now we move to the multi-device case: the model is split across several GPUs, and we run it with **pipeline parallelism (PP)**.

## Intro to PP

In pipeline parallelism, we partition the model’s layers into a sequence of chunks and place each chunk on a different device. Activations flow from the first chunk to the last, like workpieces moving down an assembly line. This lets us host models that don’t fit on a single GPU and spread the work across multiple devices.
...
### **Practical implication for decentralized PP with Macs**
